...
public partial class AudioController : SingletonMonoBehaviour<AudioController>
{
    ...
    static public AudioCategory NewCategory(AudioController aoctrler, string categoryName)
    {
        int oldCategoryCount = aoctrler.AudioCategories != null ? aoctrler.AudioCategories.Length : 0;
        var oldArray = aoctrler.AudioCategories;
        aoctrler.AudioCategories = new AudioCategory[oldCategoryCount + 1];

        if (oldCategoryCount > 0)
        {
            oldArray.CopyTo(aoctrler.AudioCategories, 0);
        }

        var newCategory = new AudioCategory(aoctrler);
        newCategory.Name = categoryName;

        aoctrler.AudioCategories[oldCategoryCount] = newCategory;
        aoctrler._InvalidateCategories();
        return newCategory;
    }
